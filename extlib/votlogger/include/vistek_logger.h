...
#endif
using namespace std;
class VISTEK_LOGGER_API vistek_logger : public std::enable_shared_from_this<vistek_logger>
{
public:
	typedef tuple<string> log_key;

	typedef std::shared_ptr<vistek_logger> ptr;
	static bool s_vistek_verbose_log;
	static std::shared_ptr<boost::asio::io_service> s_log_service;
	static std::shared_ptr<boost::asio::io_service::work> s_log_worker;
	static std::shared_ptr<boost::asio::io_service> s_print_service;
	static std::shared_ptr<boost::asio::io_service::work> s_print_worker;
...
public:
	...
	void print_args(vistek_log_type lt, T&& val, Args&&... args)
	{
		...
		process_log(false, lt, std::move(ss.str()));
	}
	...
	void input_args(vistek_log_type lt, const std::string& filename, int line, const std::string& funcName, const std::string& source, bool bWrite, T&& val, Args... args)
	{
		...
		process_log(bWrite, lt, std::move(ss.str()));
	}
...
#define vlog_trace(_src_, lt, bShowHead, bTrace, bWrite, _appendstream) { const vistek_logger::ptr& _temp_1_ = vistek_logger::get_instance(vistek_logger::log_key(_src_));\
									...
									_temp_1_->process_log(bWrite, lt, std::move(_temp_2_.str()));}}
